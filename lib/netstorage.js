...
var Netstorage = function(hostname, keyname, key, ssl) {
  if (!hostname && !keyname && !key) {
    throw new Error('You should input netstorage hostname, keyname and key all');
  }
  
  if (ssl === undefined) {
    ssl = false;
  } else if (typeof(ssl) !== 'boolean') {
    throw new TypeError('"ssl" parameter should be boolean type');
  }
...
Netstorage.prototype._request = function(kwargs, callback) {
  ...
  if (!ns_path.startsWith('/')) {
    callback(error=new Error('Invalid netstorage path'));
    return;
  }
...
Netstorage.prototype.upload = function(local_source, ns_destination, callback) {
  try {
    if (fs.lstatSync(local_source).isFile() && ns_destination.endsWith('/')) {
      ns_destination = `${ns_destination}${path.basename(local_source)}`
    } else if (fs.lstatSync(local_source).isDirectory()) {
      callback(error=new Error("You can upload a file, not a directory"));
      return;
    }
  } catch (e) {
    callback(error=e);
