    ...
    res.on('end', () => {
      ...
      if (kwargs['action'] == 'download') {
        ...
        if (kwargs['path'].endsWith('/')) {
          ...
          local_destination = path.basename(kwargs['path'])
        } else if (fs.statSync(local_destination).isDirectory()) {
          local_destination = path.join(local_destination, path.basename(kwargs['path']));
        }
        ...
        this.xml2JSON(rawData, (err, json) => {
...
Netstorage.prototype.list = function(ns_path, actionHeaders, callback) {
...
Netstorage.prototype.xml2JSON = function(xmlPayload, callback) {
  this.parser.parseString(xmlPayload, (err, results) => {
    if (err) {
      return callback(err, null)
    }

    this.parsedResults = _.mergeWith({}, results, function(a, b) {
      var obj = {};
    ...
    return callback(null, this.parsedResults)
  })
}
